...
function pushWithoutSnapshot() {
  ...
  local IMAGE_TAG2="${LAST_TAG}-${LAST_VER}.${COMMITS_AHEAD}"
  if [[ ${COMMITS_AHEAD} = "0" ]]; then
    docker build ${INPUT_BUILDOPTIONS} ${BUILDPARAMS} -t ${INPUT_REGISTRY}/${INPUT_NAME}:${IMAGE_TAG1} ${CONTEXT}
    docker push ${INPUT_REGISTRY}/${INPUT_NAME}:${IMAGE_TAG1}
  else
    docker build ${INPUT_BUILDOPTIONS} ${BUILDPARAMS} -t ${INPUT_REGISTRY}/${INPUT_NAME}:${IMAGE_TAG2} ${CONTEXT}
